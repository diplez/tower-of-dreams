...
import { Avatar, TierBadge, EmptyState } from '@/components/ui';
...
export default function RankingsScreen() {
  ...
  const fetchRankings = useCallback(async () => {
    ...
    if (data && !error) {
      ...
      // Find my rank
      if (profile) {
        const me = ranked.find((u) => u.id === profile.id);
        setMyRank(me || null);
      }
    }
    setLoading(false);
        ...
        {ordered.map((user, idx) => {
          ...
          return (
            <View key={user.id} style={[styles.podiumCard, isFirst && styles.podiumFirst]}>
              ...
              <Text style={styles.podiumFloors}>
                {user.total_floors_owned} pisos
              </Text>
            </View>
          );
        })}
  ...
  const renderRankRow = ({ item }: { item: RankedUser }) => {
    ...
    return (
      <View style={[styles.rankRow, isMe && styles.rankRowMe]}>
        <Text style={[styles.rankNum, isMe && { color: COLORS.accentGlow }]}>
          {item.position}
        </Text>
        ...
        <View style={styles.rankInfo}>
          <Text
            style={[styles.rankName, isMe && { color: COLORS.accentGlow }]}
            numberOfLines={1}
          >
    ...
    <SafeAreaView style={styles.container} edges={['top']}>
      <FlatList
        ...
        ListEmptyComponent={
          <EmptyState
            emoji="🏗️"
            title="Sin rankings aún"
            subtitle="¡Sé el primero en construir!"
          />
        }
        ListFooterComponent={
